# Election Agent
A high performance and easy-use agent service provides leader election service.

**For Developers:** *Simplify and Centralize Leader Election in Your System.*
Unburden your application code with a high-performance and user-friendly agent service for leader election. Eliminates complex implementations and leverage a reliable, centralized service for robust leader selection in your distributed systems.
...
* *Application Settings*
  **EA_ENV** - The environment of application.
...
* *Redis Settings*
  **EA_REDIS_MODE** - The redis server mode.
  Differenet modes require different URLs format, please refer to the `EA_REDIS_URLS` for details.
  Defaults to `single`.
  ...
  **EA_REDIS_MASTER** - The redis master name.
  It only take effects when the `mode` is `failover`
...
Campaign a election, returns `CampaignResult.Elected=true` if successful, , otherwise `CampaignResult.Elected=false`.
The candidate needs to set a proper `CampaignRequest.Term` in milliseconds.

When a client campaign a election successfuly once, then it needs to call `ExtendElectedTerm` to extend the elected term for holding leadership.
...
Retrived a list of pods by `GetPodsRequest.Namespace` and `GetPodsRequest.Deployment`.

The pod item represents the relationship of `Pod`, `ReplicaSet` and `Deployment`. These information helps identify which pods are "newer" through `ReplicaSet.revision` during k8s rolling update period.

The `PodStatus.terminating` helps identify whether a Pod is starting to enter graceful shutdown phase.
...
Execute `make update-tools` to install/upgdate development tools.
